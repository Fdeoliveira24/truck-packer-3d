...
**Last updated:** 2026-07-14 — BUG-01 and BUG-07 passed their automated and signed-in browser release gates; BUG-04 remains closed as a policy misdiagnosis; BUG-05 remains closed as a stale or non-reproducible observation. Multi-workspace Stripe hardening, the paid-workspace ownership-transfer guard, generic member-role ownership invariants, and F12 direct-paid workspace identity are merged, pushed, deployed, and verified against all available safe dev fixtures. Max Capacity Phase C has not started.
...
| Main baseline before this closeout | `main` and `origin/main` matched at `26ea600` after the F12 implementation/tests were fast-forward merged and pushed. |
...
| Field | Value |
|-------|-------|
| Integration branch | `main`; F12 direct-paid workspace identity is integrated through `26ea600` before this documentation-only closeout. |
| Active implementation | Complete: a requested workspace with one explicit, unambiguous usable direct binding is classified from its own subscription before owner-plan sibling coverage. |
| Current allowed dirty file during closeout | This TODO only. `billing-status` source/tests were already committed, validated, merged, pushed, and deployed. |
| Active blocker | None in the F12 implementation or available dev verification. Exact multi-direct, trialing-sibling, coupon, over-limit-sibling, duplicate, and conflicting-metadata sandbox fixtures were unavailable and are not claimed as live passes; production/function runtime coverage remains green. |
...
| Do not start simultaneously | Phase C, `getMyMembership`, future Stripe customer/subscription transfer architecture, pricing/grant work, durable-fixture work, and database constraint/reconciliation work remain separate future tasks. |

*Update this block after each commit/merge. Do not hardcode the same status in multiple conflicting places.*

> **Current source-of-truth note (2026-07-14):** Max Capacity Phase B remains complete. BUG-01 and BUG-07 passed their release gates on implementation HEAD `b0d6fb21`; BUG-04 and BUG-05 remain closed. Multi-workspace Stripe hardening is integrated through `959d306`; the ownership-transfer billing guard is integrated through `2c047a2` and deployed as `org-transfer-ownership` version 10; owner-role invariant hardening is integrated at `91ffb51` and deployed as `org-member-role-update` version 20; F12 direct-paid identity is integrated at `26ea600` and deployed as `billing-status` version 72. Phase C is not started and no Phase C branch exists.
