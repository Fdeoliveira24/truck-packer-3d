...
| ⚠️🚫 | **JSON import has no file-size gate. AUDITED 2026-06-13: fix designed + locally proven, but blocked by working-tree scope-guard debt (see 3C) — reverted to keep tree clean.** Confirmed: `import-app-dialog.js` (App import) and `import-pack-dialog.js` (Pack import) call `file.text()` with no size check, unlike the spreadsheet path (`import-export.js:103`, 10 MB cap). Designed fix: export `assertImportFileWithinSizeLimit(file)` from `import-export.js` (reusing `MAX_IMPORT_FILE_BYTES`, DRY the spreadsheet path), call it before `file.text()` in both dialogs. Added 2 behavior tests to `import-export.spec.mjs` — both passed and the existing oversized-file test stayed green. **Blocker:** changing the 3 import source files trips 23 working-tree "changed files stay inside scope" guards in `security-and-invariants.spec.mjs` (they pin each merged phase's file list against the live tree). Landing this needs the 3C scope-guard overhaul first. |
...
| Status | Item |
|--------|------|
| ⚠️❓ | **Unify trailer geometry tolerance — technical blocker for placement work. AUDITED 2026-06-13: the prescribed "change 0.01 → 0.05" fix is UNSAFE as written; needs an architecture decision before implementation.** `app.js` `TrailerGeometry.isAabbContainedInAnyZone` uses `EPS = 0.01`; `pack-library.js` and `autopack-solver.js` use `0.05` (inches). The catch: app.js's copy is a SHARED helper called in **mixed unit spaces** — inch-space at `app.js:3715` (`isInsideTruckInstance`) and `editor-screen.js:2745`, but **world-space** at `editor-screen.js:539` (passes `zonesWorld`). With `INCH_TO_WORLD = 0.05` (scene-runtime.js:24; 1 world unit = 20"), the single `EPS=0.01` already means 0.01" for inch callers but 0.2" for the world caller. Changing it to `0.05` would balloon the world-space containment check to **1.0" of slack** — weakening boundary containment (forbidden by AutoPack rules). The existing G2 audit note in `pack-library.js:209-217` already flags app.js's copy as "intentionally not changed" due to units. **Correct fix (needs decision): either make the helper unit-aware / pass an explicit per-call epsilon, or convert `editor-screen.js:539` to inch-space before checking, so every call site applies a true 0.05" tolerance.** Then add regression tests covering the epsilon boundary in both unit spaces. Requires browser verification of placement + packed/staged classification. |
...
| 🔄 | **Keep phase tests based on source ownership or behavior, not live working-tree file lists. HIGH PRIORITY BLOCKER — escalated 2026-06-13.** Done so far: retired 3 stale `git diff`-based "changed files stay inside scope" guards for merged G1.2B/G1.2C/G1.2D phases; made the remaining guards ignore the living TODO V4 planning doc. **Open problem:** ~23 of these working-tree guards remain and false-fail on ANY legitimate multi-file *source* change (each pins a merged phase's file list against the live working tree). This currently blocks otherwise-safe multi-file work (e.g. the 2B JSON import gate touches 3 import files → 23 guards fail). These guards provide zero post-merge value (they pass trivially on a clean tree). Recommended: replace each "changed files stay inside scope" assertion with behavior/source-ownership tests, or remove them, as a single focused, reviewed test-infra commit. Until then, multi-file source phases cannot validate green. |
