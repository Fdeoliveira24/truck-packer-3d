...
1. Implement Cargo Instructions Phase 1 on `feat/inspector-case-notes`: remove the modal from the Inspector's Standard Instructions surface and replace it with an always-visible read-only card, per the locked architecture in [Cargo Instructions Ownership Contract](../engineering/cargo-instructions-ownership-contract.md). This must land **before** the browser QA below, since QA against the current modal would validate a surface Phase 1 is about to remove.
2. Perform live browser QA on `feat/inspector-case-notes` (against the corrected Phase 1 surface: read-only card, top-card placement, selection safety, Transform/Rotate/Actions/Category unaffected), then push and merge to `main`.
3. Perform an `app.js` modularization inventory (M0 inventory only, not extraction) and plan low-risk extraction.
4. Cargo Instructions Phase 2 (Instance Notes) and Phase 3 (Pack Notes Editor access) — queued per the same contract; not yet approved to start.
