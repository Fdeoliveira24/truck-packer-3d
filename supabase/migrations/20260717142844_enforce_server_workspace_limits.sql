-- Packet 2: enforce owner-account workspace limits inside the same transaction
-- that creates the organization, owner membership, billing placeholder, and
-- profile selection. The Edge Function supplies the existing server catalog
-- values; only service_role can execute this RPC.

drop function if exists public.tp3d_create_workspace(uuid, text);

create function public.tp3d_create_workspace(
  p_actor_id uuid,
  p_name text,
...
revoke execute on function public.tp3d_create_workspace(uuid, text, jsonb) from public;
revoke execute on function public.tp3d_create_workspace(uuid, text, jsonb) from anon;
revoke execute on function public.tp3d_create_workspace(uuid, text, jsonb) from authenticated;
grant execute on function public.tp3d_create_workspace(uuid, text, jsonb) to service_role;
