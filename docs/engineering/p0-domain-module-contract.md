...
>
> **Stage 1 (Billing): NOT SAFE TO BEGIN STAGE 1** *(updated 2026-07-24; supersedes the earlier "SAFE TO BEGIN STAGE 1" here)*. Amendment 1's decisions (A1/A2/A3) stand, but a complete **bidirectional AST free-variable gate** (**Stage 1 Amendment 2**, end of doc) proved the A3 retarget surface incomplete: the IIFE billing pump / focus / workspace-lifecycle / auth-lifecycle code directly **reads and writes** billing-private state and calls ~14 billing-private functions. Direct writes to billing-private state from the IIFE cannot be expressed through a copy-returning facade. The Billing↔pump boundary is an unresolved architecture decision — see Amendment 2.
...
**MOVE closure-dependency summary:** every MOVE function closes over (a) billing-private state — **moves with it**; (b) the A1 injected dependencies — **provided at construction**; (c) for `getActiveOrgIdForBilling` only, the `window.OrgContext` global — **retained global read**; (d) platform globals (`localStorage`/`BroadcastChannel`/`document`/`navigator`/`window.__TP3D_USER_SWITCH_PENDING`) — **accessed directly, unchanged**. **CORRECTION (Amendment 2):** this claim was **false** — the AST free-variable pass found four additional MOVE→keeper deps (`nullableFiniteNumber`, `ORG_UUID_RE`, `_bootStartedAtMs`, `_authTruthSnapshotAccessor`, `_orgAccessLossHandler`; resolved in Amendment 2 · A), *and* a large reverse surface (IIFE→billing-private reads/writes/calls) that Amendment 2 records as a blocker. The A3 "≈48 mechanical retargets" figure counted only public billing functions and is **incomplete**.
...
**SAFE TO BEGIN STAGE 1** *(Amendment 1 — **SUPERSEDED** by Amendment 2's NOT SAFE TO BEGIN; this paragraph's gating claims about A1/A2 remain valid, but its "external retarget surface is fully enumerated" claim is **retracted** — see Amendment 2 · B)*. Amendment-1 gating: the five shared helpers have permanent ownership (RETAIN @ root + inject, A1); the `createBillingService` dependency shape is exact (A1); `applyAccessGateFromBilling` and `_billingGateApplier` have final Billing ownership with one exact setter `setBillingGateApplier(fn)` (A2); ~~the external retarget surface is fully enumerated~~ **(false — A3 was incomplete; Amendment 2 · B)**; no import cycle is required (Billing receives root utilities by injection, imports only `core`/`services`); no new public-facade member is added (`applyAccessGateFromBilling`/`setBillingGateApplier`/`getActiveOrgIdForBilling` are off-facade). **The governing Stage-1 status is Amendment 2: NOT SAFE TO BEGIN STAGE 1.**
...
> **⚠ SUPERSEDED — this "SAFE TO BEGIN STAGE 1" is retracted.** A complete **bidirectional AST free-variable gate** (Stage 1 Amendment 2, below) proved the A3 retarget surface incomplete and found the Billing↔IIFE boundary far more entangled than modelled (the IIFE **writes** billing-private state directly). The governing Stage-1 status is now **NOT SAFE TO BEGIN STAGE 1** (Amendment 2). This Amendment-1 text is retained as the (still-correct) shared-utility / gate-applier / public-facade contract, but its approval line no longer governs.

---

## Stage 1 Amendment 2 — Complete bidirectional free-variable gate (BLOCKER)

**Added:** 2026-07-24 · **Verified against** `src/app.js` @ `c7bdc7c` via **AST free-variable analysis** (espree, scope-aware), in **both** directions. Documentation-only. This amendment (a) freezes the four dependency decisions the first move-map missed, and (b) records a **larger, previously-undocumented Billing↔IIFE private-coupling surface** that makes Stage 1 **not mechanically extractable as currently scoped**. **Governing Stage-1 status: NOT SAFE TO BEGIN STAGE 1.**

### Why the earlier verifications missed this
Amendment 1 (A3) and its review enumerated the retarget surface from **caller-count greps of the *public* billing functions**. Greps cannot see (i) a moved function's internal free variables, nor (ii) references to *private* billing symbols from outside. An AST pass sees both. **Permanent lesson, now a contract gate (below): grep/reference-counting is insufficient; a scope-aware free-variable pass is mandatory before Stage 1 edits.**

### (A) MOVE → keeper free variables — RESOLVED (four decisions frozen)

The AST pass found **five** billing→keeper references the frozen `createBillingService` contract omitted. Four decisions resolve them; after resolution the (A) direction is **clean (zero unresolved)**.

| # | Symbol | Source (verified) | Decision |
|---|--------|-------------------|----------|
| D1 | `nullableFiniteNumber` (360, fn) | used **only** by `applyBillingEntitlementFields` (380–381); no non-billing caller | **MOVE — Billing-private.** Co-locate with `applyBillingEntitlementFields`; not injected, not exported, not on facade; args/NaN/null/finite behavior unchanged. Add to MOVE map. |
| D2 | `ORG_UUID_RE` (927, const) | `/^…$/i` — **`/i` only, no `/g`/`/y`, no `lastIndex` state**; used by Billing `getActiveOrgIdForBilling` (949) + retained `normalizeOrgIdForBilling` (933) + IIFE org (5544, 5648) | **RETAIN @ root; INJECT the exact RegExp reference** into `createBillingService`. No clone/reconstruct/duplicate. Single source of truth; no flag drift. Off-facade. No `lastIndex` risk (non-global `.test()`). |
| D3 | `_bootStartedAtMs` (175, const) | `Date.now()` at module-eval; used by Billing `refreshBilling` trace (1406 `ageFromInitMs`) + IIFE focus guard (9253) | **RETAIN @ root; INJECT as an immutable construction-time value** `bootStartedAtMs: _bootStartedAtMs`. Captured once; same epoch (construction is module-eval, after 175); no `Date.now()` replacement; `ageFromInitMs` identical; off-facade. |
| D4 | `_authTruthSnapshotAccessor` (1171, let; init `() => null`; assigned **5202** inside IIFE) | Billing `getCurrentBillingAuthUserId` (211) **and keeper** `ensureWorkspaceReadyForUI` (1178, 1210) both read it | **Late-bound private setter `setAuthTruthSnapshotAccessor(fn)`.** Billing stores a private accessor ref (init unset → same `typeof==='function'?…:null` no-op). Root **retains** `_authTruthSnapshotAccessor` (ensureWorkspaceReadyForUI still reads it) **and additionally** calls the setter at 5202 with the same reference. Not on facade; no immediate work/notify/refresh/gate/async; repeat-replace; returns undefined. |
| D5 | `_orgAccessLossHandler` (179, let; init `null`; assigned **6126** = `handleOrgAccessLoss`) | used **only** by Billing `refreshBilling` (1630–1631) on access-denied; **no keeper reader** | **Late-bound private setter `setOrgAccessLossHandler(fn)`.** Billing stores a private handler ref (init null → same early behavior). Root's `let _orgAccessLossHandler` (179) becomes orphaned and is removed; the 6126 assignment becomes `BillingService.setOrgAccessLossHandler(handleOrgAccessLoss)`. Invoked with exact args `(requestedOrgId, {reason,status,message})`; truthy→early-return `getBillingState()`; falsy→toast path; throw behavior unchanged. Organization keeps ownership of `handleOrgAccessLoss`. Not on facade. |

**Updated exact factory signature** (Amendment-1 A1 nine deps + two):
`createBillingService({ SupabaseClient, fetchBillingStatus, createCheckoutSession, createPortalSession, isTp3dDebugEnabled, normalizeOrgIdForBilling, normalizeBillingEntitlementStatus, billingDebugLog, billingAuthLifecycleDebugLog, ORG_UUID_RE, bootStartedAtMs })`. `ORG_UUID_RE` = shared root RegExp reference (required, captured once, off-facade); `bootStartedAtMs` = number value (required, captured once, immutable, off-facade). **Not** added: `_authTruthSnapshotAccessor`, `_orgAccessLossHandler`, `handleOrgAccessLoss` (all late-bound via setters, not construction — assigned later in the IIFE).

**Construction-timing constraint (new):** because `ORG_UUID_RE` (927) and `_bootStartedAtMs` (175) are `const` (TDZ, not hoisted), `createBillingService(...)` must be called **after** both are defined — i.e. at the existing facade point (~2103), not earlier. The hoisted function-decl helpers impose no such constraint. Channel/tab-id/listener setup then runs during construction (still module-eval, pre-init); nothing calls billing before construction.

**Updated private BillingService instance API (off-facade):** `applyAccessGateFromBilling`, `setBillingGateApplier`, **`setAuthTruthSnapshotAccessor`**, **`setOrgAccessLossHandler`**, plus the previously-frozen root-callable privates (`getActiveOrgIdForBilling`, `clearBillingAuthoritativeRefreshRequirement`, `markBillingAuthoritativeRefreshForNextSignIn`). None on `window.__TP3D_BILLING`. Wiring points: `setBillingGateApplier`@9239, `setAuthTruthSnapshotAccessor`@5202 (added alongside the retained assignment), `setOrgAccessLossHandler`@6126 (replaces the assignment).

### (B) keeper/IIFE → MOVE references — **INCOMPLETE / BLOCKER**

The reverse AST pass shows the IIFE reaches into billing **private** internals far beyond A3's ~13 accounted public functions. The billing **pump** (`maybeScheduleBillingRefresh`/`resetBillingPumpForUserSwitch`, RETAIN), the **focus/visibility refresh** handler (~9253–9296), the **workspace-lifecycle** billing reconciliation (handleWorkspaceLeft/Archived/Restored/Updated), and the **auth-lifecycle** logging/callbacks (renderAuthState, auth-callback, signed-out cleanup) directly use:

**B1 — ~14 billing-private *functions* (need private-API exposure + retarget; not in A3):** `abbreviateBillingLifecycleId` (9 sites), `clearBillingPendingRetry` (7), `_readShareableBillingResult` (4), `_applySharedBillingSnapshot` (3), `_getSharedBillingFreshness` (3), `reconcileBillingStateForActiveOrg` (3), `transferPendingPostSignoutBillingRequirementForAuthenticatedUser` (3), `_readSharedBillingResult` (2), `_shouldApplySharedBillingSnapshotForOrg` (2), `isEntitlementAllowed` (2), `getBillingAuthoritativeRefreshToken` (1), `isBillingAuthoritativeRefreshInFlight` (1), `getCurrentBillingAuthUserId` (1), `requireBillingAuthoritativeRefreshForUserSwitch` (1).

**B2 — direct *reads* of billing-private state from the IIFE:** `_billingState.*` field reads at ~11 sites (5747, 5865, 5923, 5959, 5994, 6037, 9258, 9261, 9266–9268) — **not** via the copy-returning `getBillingState()`; plus `_billingRequireAuthoritativeOnNextSignIn` (7391), `_BILLING_SHARED_FRESH_MS` (5787, 7269), `BILLING_FOCUS_REFRESH_COOLDOWN_MS` (9262, 9268, 9273), `_billingLastFocusRefreshAt` (9262).

**B3 — direct *writes* to billing-private state from the IIFE (the hard blocker):** `_billingLastFocusRefreshAt = now` (9296, focus handler); `_lastBillingKey = ''` (5674) and `_lastBillingKeyAt = 0` (5675, inside `resetBillingPumpForUserSwitch`). A copy-returning facade **cannot** express these writes; they would require **new billing state-mutation APIs** — new ownership decisions outside any frozen contract.

**Consequence.** Stage 1 as scoped (move billing state/functions to a module; keep the pump/focus/lifecycle code in the IIFE) is **not** "move + inject + wire + mechanical retarget." It additionally requires a **large private orchestration API** (~14 methods), **state-read retargets** (~11 `_billingState.*` sites → `getBillingState()` reads, which changes copy-allocation semantics and is a rewrite, not a 1:1 retarget), and **≥3 new state-mutation setters** (B3). That is an **architecture/boundary decision**, not a dependency completion — and it is explicitly out of scope here.

### Free-variable completeness gate (permanent contract addition)

Before **any** Stage 1 production edit, run a **scope-aware AST free-variable analysis in both directions** over the MOVE set. Every MOVE-function free variable and every external→MOVE reference must classify as exactly one of: *moved Billing declaration · injected construction dependency · approved retained platform global · private late-bound module dependency · imported provider · language built-in · **UNRESOLVED BLOCKER***. **Approval requires zero UNRESOLVED BLOCKERs in both directions.** Grep/reference-counting does **not** satisfy this gate (it misses internal closure deps and private-symbol external use — exactly how B1/B2/B3 went undetected through two prior amendments). Current gate result: **(A) clean; (B) UNRESOLVED — B1 (14 private fns), B2 (private-state reads), B3 (private-state writes).**

### Approval — Amendment 2

**NOT SAFE TO BEGIN STAGE 1.** Decisions D1–D5 are frozen and correct, but the (B) surface (B1/B2/B3) is an unresolved boundary decision: the IIFE billing pump / focus / workspace-lifecycle / auth-lifecycle code directly reads and **writes** billing-private state and calls ~14 private billing functions. **Minimal recommendation (architect decision required, then a scoped amendment):** choose the Billing↔pump boundary — either **(a)** move the pump + focus-refresh + workspace-lifecycle billing-reconciliation + billing-lifecycle logging into `billing-service.js` (accepting that this code also reads org/auth/IIFE state, which would then need its own injection contract), or **(b)** define the full private orchestration API + explicit billing state-mutation setters for B1/B2/B3 and re-run the bidirectional gate to zero. Do **not** invent either mid-extraction on the money path.
