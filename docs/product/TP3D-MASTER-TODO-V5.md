...
12. Billing reliability, the Platform UX/UI Compatibility Closeout, Max Capacity Phase C reporting, the evidence-only AutoPack strategy differentiation audit, the evidence-only legacy AutoPack solver retirement audit, and the legacy AutoPack solver removal are closed; Phase C was merged to `main` at `99be0776d0070f18b18379bbe1e978a3dec03c43`, the strategy audit was merged to `main` at `ec1cf4a`, and the legacy solver removal was merged to `main` at `716c4d1`. The `app.js` P0 modularization is complete and human-approved on `refactor/app-js-p0-domain-cluster` (`bc9c6fd`), a combined release branch that also carries Cargo Instructions Phase 0/1/2 (live QA complete) and Inspector case notes; the whole branch was merged to `main` via combined PR #7 (`1199711`; the separate `feat/inspector-case-notes` merge is obsolete). Cargo Instructions Phase 3 (Section 4) is the single active task.
...
| Cargo Instructions Phase 1 — Standard Instructions | Implementation and automated validation complete (committed at `ffd1d75`; now carried on the combined release branch `refactor/app-js-p0-domain-cluster`). Supersedes the earlier Inspector Notes modal prototype: `case.notes` is now rendered as an always-visible, read-only "Standard Instructions" section in the Inspector's top Case summary card (no modal, no edit affordance); the Cases-screen Case editor (`case-modal.js`) remains the only editing location. Persisted field name and normalization unchanged. Live browser QA complete on the combined release branch `refactor/app-js-p0-domain-cluster` (authenticated Chrome): read-only Standard Instructions card, correct per-case content with no stale-on-selection, top-card placement, and Transform/Rotate/Actions/Category unaffected. No AutoPack, solver, billing, auth, or workspace behavior changed. |
| Cargo Instructions Phase 2 — Instance Notes | Implementation and automated validation complete (committed at `086004b`; now carried on the combined release branch `refactor/app-js-p0-domain-cluster`). Adds Pack-instance-owned `instanceNotes` (string-or-null, same normalization rule as `case.notes`) via `normalizer.js::normalizeInstance`; Inspector single-selection top summary card gets an "Item Notes" control using the same proven Empty/Read/Edit modal pattern, saved exclusively through `PackLibrary.updateInstance` with packId/instanceId re-resolved before every Save. Also fixed a real duplication gap found while verifying the Phase 0 audit: `buildSafeDuplicateInstances` was deep-cloning the full source instance, which would have silently copied `instanceNotes` onto a fresh duplicate; a duplicated instance now explicitly starts with `instanceNotes: null`. Live browser QA complete on the combined release branch `refactor/app-js-p0-domain-cluster` (authenticated Chrome): create/edit/save/clear, full-reload persistence, correct packId/instanceId ownership, and no note leak across instances; the data-layer survival matrix (AutoPack/Truck-Change/duplication/undo-redo/export-import) is green in the automated suite. No AutoPack, solver, billing, auth, or workspace behavior changed. |
...
| Field | Current value |
|---|---|
| Task | **Business Identifiers Phase 0 — Product and Data Contract.** A decision and architecture contract only: define which tiers carry a business identifier (Case, Pack/Load Plan, packed instance), generation and editability, uniqueness scope, duplicate and import-conflict behavior, external reference fields (SKU/PO/shipment/ERP/WMS), export/manifest/label surfaces, migration policy, and barcode readiness. **No implementation, no schema migration, and no UI in this phase.** |
| Branch | Not yet created. |
| Outcome | Not yet started. |
| Blocker state | Unblocked. Its prerequisites are met: Cargo Instructions Phase 3 is merged (PR #9, `d0ae772`) and the customer-facing naming model is settled (PR #10 — see the Customer-Facing Naming contract in Section 8), so identifiers can be named against a stable vocabulary instead of the former Pack/project/load ambiguity. |
| Scope boundary | Contract and decisions only. Identifiers must stay separate from internal UUIDs, from `pack.id`, from physical packing signatures, and from the workspace slug — slug possession must never act as an access credential. Do not couple this phase to Pack Publishing / share links, billing, auth, workspace, or AutoPack behavior. |
...
1. Business Identifiers Phase 0 — Product and Data Contract (Section 4). Evidence and decisions only; it produces a contract document, not code. Implementation phases require separate approval after Phase 0 closes.

(The combined release branch `refactor/app-js-p0-domain-cluster` — `app.js` modularization + Cargo Instructions Phase 0/1/2 + Inspector case notes + stabilization — was merged to `main` via PR #7 at `1199711`; see Section 7.)
...
- No open blockers. The Cargo Instructions track is **closed**: Phase 1/2 live browser QA completed 2026-07-27 and Phase 3 merged via PR #9 at `d0ae772`. The customer-facing naming decision is settled and shipped via PR #10, which was the outstanding prerequisite for Business Identifiers.
...
|---|---|---|
| Customer-facing Load Plan terminology alignment | Complete and merged via PR #10. "Load Plan" replaced "Pack" as the customer-facing business object across navigation/topbar/screen heading, the Load Plans screen (create/edit/rename/duplicate/delete, search, empty and filtered-empty copy, bulk selection, folder filter/move, every lifecycle toast), the Editor ("Load Plan Notes" toolbar action, modal, empty state, missing-record error, empty-editor CTA), the Cases-screen reference counting Load Plans per Case, import/export dialogs and validation copy, the Help modal, Settings backup copy, and the 404 overlay; plus the Untitled/Demo and export file-name fallbacks. Deliberately preserved: Auto-Pack, Unpack, repack, packed/"packed case", the `Packed` column, "packing rules", the historical 1.0.0 release-notes entry, and "Truck Packer 3D" branding (deferred to launch prep). Internal architecture untouched — `PackLibrary`, `pack.id`, `pack.notes`, `currentPackId`, DOM ids (`btn-pack-notes`), CSS classes, storage keys, route identifiers and the JSON contract (`pack`, `packs`, `packLibrary`, `pack-batch`) are unchanged, with no `loadPlanId`/`loadPlanNotes` and no compatibility wrapper. Product diff was 160 insertions / 160 deletions — pure string substitution. Validation: audit 1164 passed / 0 failed / 5 skipped (1154 baseline + 10 new), ESLint 0 errors with the 19 pre-existing warnings unchanged, `git diff --check` clean. Live authenticated desktop QA (workspace `test1`) plus a 375px structural pass with DOM measurements. | PR [#10](https://github.com/Fdeoliveira24/truck-packer-3d/pull/10), `tests/audit/load-plan-terminology.spec.mjs`, `index.html`, `src/screens/packs-screen.js`, `src/screens/editor-screen.js`, `src/ui/app-shell.js` |
| Cargo Instructions Phase 3 — Pack Notes Editor access | Complete and merged to `main` via PR #9 at `d0ae772` (feat commit `95bc315`). Adds a `btn-pack-notes` viewport-toolbar action available whenever a Pack is loaded (never gated on instance selection, disabled only on no-pack or an in-flight editor operation) and `openPackNotesModal(pack)`, a notes-only empty/read/edit modal reusing the proven `tp3d-notes-modal` pattern. Saves exclusively through `PackLibrary.update(packId, { notes })`, with packId captured up front, re-resolved before every Save, and `editorMutationBlocked()` re-checked before the write; never touches `case.notes`, `instanceNotes`, `CaseLibrary` or `updateInstance`. No data-layer field added — `pack.notes` already existed and already flowed to PDF export. Completes the three-tier Cargo Instructions model. 9 new tests; audit 1154 passed / 0 failed / 5 skipped, ESLint 0 errors, `git diff --check` clean, plus live authenticated Chrome QA (open with no selection, empty/read/edit/save/cancel/clear, full-reload persistence, exactly one modal, no console/network errors). | PR [#9](https://github.com/Fdeoliveira24/truck-packer-3d/pull/9), `index.html`, `src/screens/editor-screen.js`, `tests/audit/inspector-case-notes.spec.mjs` |
| `app.js` P0 modularization + Cargo Instructions Phase 1/2 QA closeout | Complete and human-approved on the combined release branch `refactor/app-js-p0-domain-cluster` (modularization-cleanup `bc9c6fd`). Extracted Billing, Organization, Auth, and AccountSwitcher from `app.js` into dedicated frozen modules; the Composition Root feasibility review closed as a no-op by design; final dead-code cleanup + regression audit. `app.js` reduced 9,510 → 6,867 lines (−2,643, −27.8%). Cargo Instructions Phase 1/2 live browser QA completed in an authenticated Chrome session (read-only Standard Instructions card; Item Notes create/edit/save/clear, full-reload persistence, per-instance ownership, selection safety). The branch is a combined release branch (stacked base: Cargo Phase 0/1/2, Inspector case notes, stabilization) and was merged to `main` via combined PR #7 (merge commit `1199711`) — `feat/inspector-case-notes` was not merged separately. Validation: audit 1145/0/5, ESLint 0 errors, `git diff --check` clean. | combined PR [#7](https://github.com/Fdeoliveira24/truck-packer-3d/pull/7), merged to `main` at `1199711` (branch `refactor/app-js-p0-domain-cluster`, `bc9c6fd`) |
| Cargo Instructions Phase 0 — architecture lock | Closed. Evidence-only; no production code changed, no user-visible behavior changed. Locks the permanent three-tier ownership contract: Standard Instructions (Case-owned, read-only in Inspector, no modal — supersedes the modal-based read-only surface built on this branch), Instance Notes (new, Pack-instance-owned, field name `instanceNotes` to avoid collision with `Case.notes`), Pack Notes (already exists, Pack-owned, needs only an Editor access point). Validated against the current codebase, not assumed: confirmed the "Case edits propagate live to every Pack Instance" policy already holds today (instances hold only `caseId`, never a snapshot; `renderSingleInspector` re-resolves `CaseLibrary.getById` fresh every render; `src/app.js`'s global `StateStore.subscribe` calls `EditorUI.render()` on every `caseLibrary` change), and confirmed a future `instanceNotes` field would survive AutoPack repacking, Truck Change reconciliation, and Pack duplication for free (every instance-rewriting code path deep-clones-and-patches rather than reconstructing from a narrow field list). Full ownership diagram, data contract, impact report, risk assessment, and phased implementation plan recorded in the linked contract doc. | [Cargo Instructions Ownership Contract](../engineering/cargo-instructions-ownership-contract.md) |
| Cargo Instructions Phase 1 — Standard Instructions | **Implementation, automated validation, and live browser QA all complete (authenticated Chrome, 2026-07-27).** Committed at `ffd1d75`, now shipping via the combined release branch `refactor/app-js-p0-domain-cluster`. Per the Phase 0 architecture lock, removed the Inspector Notes modal prototype (`openCaseNotesModal` and its button) entirely and replaced it with an always-visible, read-only "Standard Instructions" section in the Inspector's top Case summary card, restoring the header to its pre-feature structure. The Cases-screen Case editor (`case-modal.js`) remains the sole editing location; its label was updated to match ("Standard Instructions" / "Applies to every unit of this Case."). Persisted field (`case.notes`) and its normalization are unchanged. 17 focused tests (data-layer unchanged from Phase 0's prior work, plus new static-render/absence source-contract checks replacing the obsolete modal-editing-contract tests); full suite 1104/1104 passed (0 fail, 5 pre-existing skips), typecheck and lint clean. **Live browser validation complete** (authenticated Chrome): read-only card render, correct per-case content, top-card placement, selection safety, Transform/Rotate/Actions/Category unaffected. | `src/screens/editor-screen.js`, `src/ui/overlays/case-modal.js`, `styles/main.css`, `tests/audit/inspector-case-notes.spec.mjs` |
| Cargo Instructions Phase 2 — Instance Notes | **Implementation, automated validation, and live browser QA all complete (authenticated Chrome, 2026-07-27).** Committed at `086004b`, now shipping via the combined release branch `refactor/app-js-p0-domain-cluster`. Adds `instanceNotes` (string-or-null, same normalization rule as `case.notes`) to `normalizer.js::normalizeInstance`. Inspector single-selection top summary card gets an "Item Notes" control reusing the proven Empty/Read/Edit modal pattern, saved exclusively through `PackLibrary.updateInstance(packId, instanceId, { instanceNotes })`; packId/instanceId captured at open time and re-resolved before every Save, failing safely (toast, draft preserved) if either no longer exists. Verified directly (not assumed): AutoPack repacking and Truck Change/canonical-pose reconciliation both already preserve `instanceNotes` via existing spread/deep-clone patterns — no change needed there. Found and fixed one real gap: `PackLibrary.buildSafeDuplicateInstances` deep-clones the full source instance, which would have silently copied `instanceNotes` onto a freshly duplicated instance; it now explicitly resets to `null` on duplicate, mirroring the existing `packedProfile`-clearing precedent beside it. Whole-Pack duplication correctly preserves `instanceNotes` on every cloned instance (no change needed). 16 new focused tests (normalization, mutation isolation, undo/redo, ownership independence across two instances of one Case, both duplication paths, AutoPack/Truck-Change preservation, JSON round trip, source-contract checks); full suite 1120/1120 passed (0 fail, 5 pre-existing skips), typecheck and lint clean (0 new warnings). **Live browser validation complete** (authenticated Chrome): create/edit/save/clear, full-reload persistence, packId/instanceId ownership, and no cross-instance note leak; survival matrix green in the automated suite. | `src/core/normalizer.js`, `src/screens/editor-screen.js`, `src/services/pack-library.js`, `tests/audit/inspector-case-notes.spec.mjs` |
...
- **"Cargo Planner 3D"** is the provisional product name. It is **not yet applied in the UI**: the shipped brand mark, `<title>`, error/maintenance overlays and trial copy still read "Truck Packer 3D". Final branding, domain, legal pages, and a dedicated product email are deferred until launch preparation.
- **"Load Plan"** is the customer-facing name of the business object. Customer-visible copy — navigation, headings, forms, labels, search, empty states, confirmations, toasts, tooltips, menus, help, and exports — must say Load Plan.
- **"Pack" remains the internal technical term** and the name of the **Auto-Pack** action. `PackLibrary`, `pack`, `pack.id`, `pack.notes`, `currentPackId`, `packLibrary`, DOM ids, CSS classes, storage keys, route identifiers, file names and the JSON schema keys (`pack`, `packs`, `packLibrary`, `pack-batch`) are internal and must not be renamed for terminology reasons.
- Never introduce a parallel `loadPlanId`, `loadPlanNotes`, or any compatibility wrapper duplicating a Pack field.
- Preserve packing action/process vocabulary: Auto-Pack, Auto-Packing, Unpack, repack, packed item / packed case, and packing rules.
- Historical release notes may keep older terminology when it accurately describes a prior product state.
- `support@pxl360.com` is the real, monitored support address.
...
**Phase 0 (Product and Data Contract) has been promoted to Active Work — see Section 4.** Only that decision/contract phase is approved; every implementation phase remains deferred and requires separate approval after Phase 0 closes.

- Case, Pack, and instance business identifiers remain separate from internal UUIDs and physical packing signatures.
- Exact fields, uniqueness, number formats, import/search behavior, and migration policy are exactly what Phase 0 must decide.
- Identifiers must never be coupled to the workspace slug; slug possession must never grant access.
