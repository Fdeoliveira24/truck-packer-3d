...
**Last verified repository state:** Platform-foundation reliability and the final Platform UX–UI Compatibility Closeout are complete; Max Capacity Phase C reporting is closed and merged to `main` at `99be0776d0070f18b18379bbe1e978a3dec03c43`; the evidence-only AutoPack strategy differentiation audit is closed and merged to `main` at `ec1cf4a`, with no production behavior change; the legacy AutoPack solver retirement audit is closed with a **SAFE TO REMOVE AFTER SMALL DEPENDENCY CLEANUP** disposition, and the legacy solver removal is merged to `main` at `716c4d1`, with no production behavior change. Cargo Instructions Phase 1 (Standard Instructions — read-only Inspector card, replacing the earlier Inspector Notes modal prototype) and Phase 2 (Instance Notes — Pack-instance-owned `instanceNotes`) are both implementation-complete, automated-test-validated, and committed on `feat/inspector-case-notes` at `086004b` (not yet pushed or merged; commits `ffd1d75` and `086004b`). Live browser QA for both phases has not been performed (see Section 6) and remains outstanding before this branch merges. Phase 3 (Pack Notes Editor access), business identifiers, and the `app.js` modularization inventory have not been started, per explicit scope boundaries on the current execution.
...
12. Billing reliability, the Platform UX/UI Compatibility Closeout, Max Capacity Phase C reporting, the evidence-only AutoPack strategy differentiation audit, the evidence-only legacy AutoPack solver retirement audit, and the legacy AutoPack solver removal are closed; Phase C was merged to `main` at `99be0776d0070f18b18379bbe1e978a3dec03c43`, the strategy audit was merged to `main` at `ec1cf4a`, and the legacy solver removal was merged to `main` at `716c4d1`. Cargo Instructions Phase 1 and Phase 2 are implementation-complete and committed on `feat/inspector-case-notes` at `086004b` (not yet pushed or merged; live browser QA outstanding — Section 6). An `app.js` modularization inventory (Section 4) is the single active task.
...
|---|---|
| Repository | `main` is at `716c4d1` (legacy-solver removal merged; `src/services/autopack-legacy-solver.js` deleted, dependent tests/docs corrected; no production behavior changed). `feat/inspector-case-notes` holds Cargo Instructions Phase 1 (`ffd1d75`) and Phase 2 (`086004b`), both committed, unpushed, and automated-test-validated; live browser QA is outstanding for both. An `app.js` modularization inventory (M0 only) remains the next active task, not yet started. |
...
| Cargo Instructions Phase 1 — Standard Instructions | Implementation and automated validation complete on `feat/inspector-case-notes` at `ffd1d75` (not yet pushed or merged). Supersedes the earlier Inspector Notes modal prototype: `case.notes` is now rendered as an always-visible, read-only "Standard Instructions" section in the Inspector's top Case summary card (no modal, no edit affordance); the Cases-screen Case editor (`case-modal.js`) remains the only editing location. Persisted field name and normalization unchanged. Not yet marked fully closed: live browser QA has not been performed (no dev-serve/login capability in the working environment — confirmed directly, see Section 6). No AutoPack, solver, billing, auth, or workspace behavior changed. |
| Cargo Instructions Phase 2 — Instance Notes | Implementation and automated validation complete on `feat/inspector-case-notes` at `086004b` (not yet pushed or merged). Adds Pack-instance-owned `instanceNotes` (string-or-null, same normalization rule as `case.notes`) via `normalizer.js::normalizeInstance`; Inspector single-selection top summary card gets an "Item Notes" control using the same proven Empty/Read/Edit modal pattern, saved exclusively through `PackLibrary.updateInstance` with packId/instanceId re-resolved before every Save. Also fixed a real duplication gap found while verifying the Phase 0 audit: `buildSafeDuplicateInstances` was deep-cloning the full source instance, which would have silently copied `instanceNotes` onto a fresh duplicate; a duplicated instance now explicitly starts with `instanceNotes: null`. Not yet marked fully closed: live browser QA has not been performed (same environment blocker as Phase 1). No AutoPack, solver, billing, auth, or workspace behavior changed. |
...
| Blocker state | Unblocked. Cargo Instructions Phase 1 and Phase 2, the tasks that previously preceded this in the queue, are committed on `feat/inspector-case-notes` at `086004b`; that branch's own remaining step (live browser QA, then push/merge) does not block starting this inventory. |
...
1. Perform live browser QA on `feat/inspector-case-notes` covering both committed Cargo Instructions surfaces: Phase 1's read-only Standard Instructions card (top-card placement, populated/empty states, selection safety, Transform/Rotate/Actions/Category unaffected) and Phase 2's Item Notes modal (packId/instanceId selection safety, ownership independence across two instances of the same Case, persistence, AutoPack/Truck Change/duplication). Then push and merge to `main`.
2. Perform an `app.js` modularization inventory (M0 inventory only, not extraction) and plan low-risk extraction.
3. Cargo Instructions Phase 3 (Pack Notes Editor access) — queued per the locked architecture in [Cargo Instructions Ownership Contract](../engineering/cargo-instructions-ownership-contract.md); not yet approved to start. Business identifiers (Section 12) remain a separate, later decision — not part of this queue item.
...
- Cargo Instructions Phase 1 (`ffd1d75`) and Phase 2 (`086004b`) are implementation-complete, automated-test-validated, and committed on `feat/inspector-case-notes`. Live browser QA for both is outstanding and blocks push/merge of this branch: this environment has no dev-serve script and no Supabase credentials. Confirmed directly this session — a local static file server reaches the sign-in screen, but no `.env`/credentials exist in the repository to authenticate past it into the Editor. This does not block the `app.js` modularization inventory (a separate, not-yet-created branch) from starting.
...
The evidence-only legacy AutoPack solver retirement audit is closed and merged to `main` at `94dbfe4` with disposition SAFE TO REMOVE AFTER SMALL DEPENDENCY CLEANUP. Following that disposition, `src/services/autopack-legacy-solver.js` was deleted and merged to `main` at `716c4d1`; `buildLegacyAutoPackItems` is unaffected and remains owned by `autopack-item-builder.js`; no solver, engine, or editor behavior changed. Cargo Instructions Phase 1 and Phase 2 are implementation-complete and committed on `feat/inspector-case-notes` at `086004b` (live browser QA outstanding for both); an `app.js` modularization inventory (Section 4) is now the single active task. See Section 7 for evidence links.
