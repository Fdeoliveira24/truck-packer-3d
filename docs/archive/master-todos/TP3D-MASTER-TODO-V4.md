...
### Urgent Truck Change staged-reconciliation hotfix — merged and pushed (2026-07-13)
- ✅ First bad commit: `9138e34868f2498684b3977845e6a6bac1d90ec7` (`fix(staging): normalize and group staged case layouts`). The later editor visual-highlight commit `6d91381` is not causal and remains intact.
- ✅ Root cause: Truck Change treated the finite manual staging work-area rectangle as a generated-layout capacity limit. In the 521-case reproduction, 154 staged cases happened to remain inside that rectangle while 367 physically safe organized staged cases extended beyond its maximum Z, entered `stagedAdjusted`, and were then rejected because their grouped band could not fit the same compact rectangle.
- ✅ Hotfix on `hotfix/truck-change-staged-reconciliation-regression`: generated organized staging uses a supported canonical corridor whose X span covers the longest curated trailer preset and whose positive-Z row depth can expand deterministically. The finite manual work area remains unchanged. Hard checks for truck/blocked-body intersection, overlap, grounding, canonical dimensions, and unsupported coordinate drift remain active.
- ✅ Correction ownership is two-pass: every byte-equivalent safe original staged AABB is reserved first, then only genuine hazards are grounded/relocated against those reservations. This prevents early repairs from occupying later safe rows and cascading hundreds of false corrections; grouped correction bands still preserve Contract B ordering, separation, preview/commit parity, and semantic counts.
- ✅ Large-load proof: Standard ↔ Wheel Wells, Standard → Front Overhang, Front Overhang → Wheel Wells, and the sequential 53 ft → 26 ft → Sprinter preset chain all preserve 521 safe staged cases with `521 existing staged items unchanged`, `0` unsafe corrections, successful Apply, stable organized rows, and `0` console errors. The exact `Test2-Packs` pack was restored to Custom/Standard `1615.4 × 259.1 × 279.4 cm`, with 521 staged and 0 packed.
- ✅ Genuine-correction proof: a width expansion adjusts only the 20 staged AABBs that actually intersect the proposed truck while 501 later safe cases remain byte-equivalent; a 521-case floating correction set expands beyond the preferred work area without overlap, floating, usable/blocked-body penetration, or nondeterminism. Unresolved references still receive no fabricated geometry.
- ✅ Final validation: security/invariants `784 passed, 5 skipped, 0 failed` (`789` total); manual/editor visual regressions `59/59`; AutoPack Results `31/31`; full suite `897 passed, 5 skipped, 0 failed` (`902` total). Syntax, typecheck, and diff checks passed; lint reported `0` errors with the existing `33` JavaScript and `18` HTML warnings. Browser smoke PASS.
- ✅ No regression was observed in Contract B behavior or the editor visual-highlight cleanup during automated validation or the exact 521-case browser matrix.
- ⬜ Deferred future audit: staging originating from a custom truck longer than the largest curated preset may lose its original X-corridor provenance after multiple sequential shrinks. This non-blocking edge is intentionally outside the urgent hotfix.
- ✅ Merge/push evidence: implementation and tests are at `38a5b56`; the documentation commit and feature integration tip are at `bccd091`. The hotfix fast-forwarded into `main` without conflicts, and `main` matched `origin/main` at `bccd091` immediately after the feature push.
...
| 🔄 | Portal manual sign-off: current mapped deep-link, unmapped-org, non-owner, and foreign-org paths passed after organization-strict deployment; exact same-customer sibling and stale mapped-subscription fixtures remain unavailable | `7455bdb`; sandbox 2026-07-14 |
...
- ✅ **Multi-workspace Stripe hardening is complete:** `fix/portal-org-strict-resolution`, `fix/checkout-race-guard-metadata`, and `fix/billing-status-unmapped-candidates` were independently reviewed, sequentially merged/pushed, deployed, and exercised against the available Stripe test-mode fixtures. Exact unavailable fixtures remain recorded below.
- ✅ Phase boundary preserved: Max Capacity Phase B remains closed; Phase C has not started, and no Phase C branch exists.

##### Multi-workspace Stripe hardening — ✅ merged, pushed, and deployed; sandbox PASS WITH FIXTURE GAPS (2026-07-14)
- ✅ **Portal organization-strict resolution:** branch `fix/portal-org-strict-resolution`, implementation/rebased commit and merge tip `7455bdb`. `stripe-create-portal-session` deployed successfully. Customer resolution is now limited to the requested organization's `billing_customers` row or organization-scoped subscription projection. User-only `stripe_customers`, customer-only subscription lookup, and newest/oldest/first sibling-subscription fallbacks were removed. A plain portal fallback is allowed only for the customer explicitly resolved through the requested organization. Available sandbox checks passed: mapped portal flow opened the correct Stripe test-mode subscription flow; unmapped organization returned HTTP 409 `no_billing_mapping_for_organization`; non-owner and foreign-organization callers returned 403; tested errors exposed no raw Stripe/PostgREST detail.
- ✅ **Checkout metadata race guard:** branch `fix/checkout-race-guard-metadata`, original commit `f8b90b0`, rebased implementation/merge tip `aa8cd45`. `stripe-create-checkout-session` deployed successfully. Explicit organization metadata/mapping remains the multi-workspace duplicate blocker; a metadata-less active subscription cannot block or redirect a multi-workspace owner. The existing legacy compatibility path remains available only behind the strict exactly-one-valid-organization owner gate. Price allow-listing, organization-scoped idempotency, Checkout Session metadata, and `subscription_data` organization metadata remain intact. Available sandbox checks passed: the mapped active organization resolved to its own Stripe test portal flow, invalid price returned 400, and a non-owner returned 403.
- ✅ **Billing-status unmapped candidates:** branch `fix/billing-status-unmapped-candidates`, original commit `a4cbe34`, rebased implementation/final pre-documentation main tip `959d306`. `billing-status` deployed successfully. The oldest-owner-workspace guess is removed; unmapped candidates are excluded before direct/included entitlement selection; a member's personal legacy subscription cannot grant another owner's organization; mapped owner-plan `included_in_plan` and `workspace_limit_reached` behavior remains intact. Available API/browser checks covered active direct, included sibling, over-limit, trial-expired, owner-subscription-required, and archived/billing-unavailable states across the disposable accounts; selected interval/direct identity stayed tied to the explicitly mapped subscription; no cross-workspace contamination or raw error detail was observed.
...
- ✅ **F12 direct-paid workspace identity is closed.** Organization-specific direct payment identity is resolved before inherited owner-plan coverage; each unambiguous directly paid requested workspace uses its own subscription data. See the dedicated evidence below.
...
- ⬜ **Still open and separate:** durable billing fixtures, plan/price catalog and unknown-price diagnostics, entitlement grant sources, composite indexes/database constraints and reconciliation, coupons, lifetime/complimentary grants, additional pricing tiers, `getMyMembership` multi-workspace resolution, future Stripe customer/subscription transfer architecture, and Max Capacity Phase C.

##### Generic member-role ownership invariant — ✅ merged, pushed, deployed; dev A–G PASS (2026-07-14)
- ✅ **Closed bypass:** before `91ffb51`, `org-member-role-update` accepted `owner`, could create a second owner-role membership or demote an existing owner after a non-transactional owner count, and never synchronized `organizations.owner_id`. That generic path could therefore bypass `org-transfer-ownership`, its locked RPC, and the paid-workspace billing guard.
- ✅ **Server contract:** the generic endpoint now permits only owner-authorized `member → admin`, `admin → member`, and same-role no-op updates. `member/admin → owner`, `owner → admin/member`, any `nextRole = owner`, any target whose membership role is owner, and the canonical `organizations.owner_id` target return HTTP 409 `{ "error": "ownership_change_requires_transfer" }` without a role update.
- ✅ **Authority and concurrency:** `organizations.owner_id` is checked directly and remains authoritative. The obsolete count-then-demote owner path and its zero-owner race are removed. Permitted updates remain organization-scoped and include the previously read role as a concurrency predicate; a vanished or concurrently changed target cannot produce `{ ok: true, member: null }`.
- ✅ **Existing authorization preserved:** owners can manage member/admin roles; admins cannot promote members to admin, edit admins, or modify an owner; non-managers remain rejected. UUID validation runs before database access, and unexpected database failures return only sanitized `member_role_update_failed` responses while detailed diagnostics remain server-side.
- ✅ **UI/client contract:** Settings Members no longer offers `owner` as a generic dropdown role. Owner rows direct users to **Transfer Ownership**, the existing transfer confirmation remains the only ownership UI, and `ownership_change_requires_transfer` maps to “Ownership changes must use Transfer Ownership.”
- ✅ **Implementation and scope:** branch `fix/owner-role-update-invariants`; implementation/tests commit and pre-documentation integration tip `91ffb511af390c25f4dd34b1a9e75d2b88d1b73b`. Files: `supabase/functions/org-member-role-update/index.ts`, `src/data/services/billing.service.js`, `src/ui/overlays/settings-overlay.js`, and `tests/audit/security-and-invariants.spec.mjs`. The transfer function/RPC/migrations, billing-status, portal, checkout, webhook, Stripe configuration, database schema, AutoPack/editor, and Phase C were not changed.
- ✅ **Automated evidence:** security/invariants `868` total (`863` passed, `5` skipped, `0` failed); manual placement `59/59`; AutoPack Results `31/31`; full suite `981` total (`976` passed, `5` skipped, `0` failed). Edge/JavaScript/test syntax, typecheck, and diff checks passed; lint reported `0` errors with existing warnings only. New evidence includes production Edge-handler runtime cases, production client-helper runtime coverage, and narrow source-contract checks.
- ✅ **Deployment:** only `org-member-role-update` was deployed to Supabase project `yduzbvijzwczjapanxbd` (`cargo-planner-3d-dev`). Version `20` is ACTIVE with source SHA `baea4e493f4db93d3b6129f979f6b0ef513264dffb53bd9126f06fc2225ca155`. No migration or other Edge Function was deployed.
- ✅ **Dev A/B:** on a disposable active-billing workspace, owner-driven `member → admin` and `admin → member` both returned HTTP 200; each resulting role was read back and `organizations.owner_id` remained byte-equivalent.
- ✅ **Dev C/D:** `member → owner` and canonical `owner → admin/member` each returned HTTP 409 `ownership_change_requires_transfer`; membership roles and canonical owner remained unchanged.
- ✅ **Dev E:** the active-billing organization could not create a replacement owner through the generic endpoint. The dedicated `org-transfer-ownership` request still returned HTTP 409 `workspace_has_active_billing`, proving the paid-transfer guard remains authoritative.
- ✅ **Dev F/G:** an admin could not manage an admin or owner; malformed organization/user IDs returned safe HTTP 400 validation errors before database mutation.
- ✅ **Fixture restoration:** the temporary disposable member was returned to `member`, removed after verification, and confirmed absent. The canonical owner remained unchanged. No billing row, Stripe object, webhook record, migration, production/customer record, or unrelated database row was changed.
- ⬜ **Still open and separate:** plan/price catalog normalization and unknown-price diagnostics; entitlement grant sources; coupons; additional tiers; lifetime/complimentary grants; database constraints and webhook reconciliation; durable billing fixtures; `getMyMembership` multi-workspace resolution; future Stripe customer/subscription transfer architecture; and Max Capacity Phase C.

##### F12 direct-paid workspace identity — ✅ merged, pushed, deployed; sandbox PASS WITH FIXTURE GAPS (2026-07-14)
- ✅ **Root cause and correction:** `billing-status` previously ran `ownerEntitlementCandidate` inclusion/limit classification before the requested organization's own direct active/trialing branches. A sibling owner-plan winner could therefore report a directly paid requested workspace as `included_in_plan` or `workspace_limit_reached` and overwrite its interval, period, price-derived limit, and subscription diagnostics. The direct requested-org binding now resolves first; owner-plan coverage runs only when the requested workspace has no valid direct grant.
- ✅ **Direct-binding contract:** a direct result requires a usable active/trialing/grace subscription with explicit requested-organization scope, a resolved Stripe subscription ID, and agreement among the selected subscription, organization-scoped projection, `billing_customers` mapping, and authoritative Stripe metadata when present. Repeated projections of the same subscription ID are deduplicated. Multiple usable scoped rows, distinct direct IDs, an unresolved competing row, conflicting local/Stripe organization binding, or one subscription ID mapped to multiple organizations is ambiguous and cannot win by date, period, or row order.
- ✅ **Direct data ownership:** an unambiguous direct requested workspace takes its status, subscription identity, price ID, interval, current period end, trial end, and workspace limit from its own subscription. Later owner metadata fallback is explicitly disabled on that path. Unknown but explicitly mapped active prices retain paid access under the existing fallback; amount and coupon representation are not entitlement authority.
- ✅ **Ambiguity safety:** ambiguous direct mappings return the existing compatible fail-closed result `owner_subscription_required`, set `duplicateActiveMappings: true`, expose no arbitrary interval/period/trial/payment winner, and skip owner billing-customer active synchronization. No new entitlement enum was added.
- ✅ **Sibling coverage and diagnostics:** unpaid requested workspaces still use the existing owner-plan `included_in_plan` / `workspace_limit_reached` logic and oldest-slot policy. Production/test search found no consumer of `includedOrgIds` outside the `billing-status` debug response, so it remains a diagnostics-only owner-plan coverage list rather than being rewritten to `[resolvedOrgId]`; requested direct identity comes from the top-level direct subscription fields.
- ✅ **Implementation/deployment:** branch `fix/billing-status-direct-paid-identity`; implementation/tests and pre-documentation main tip `26ea6004761d1293c477de89390bc46727863d29`. Only `supabase/functions/billing-status/index.ts` and `tests/audit/security-and-invariants.spec.mjs` changed. Only `billing-status` was deployed to Supabase project `yduzbvijzwczjapanxbd` (`cargo-planner-3d-dev`); version `72` is ACTIVE with source SHA `d05b52c2f4b4d8cb61d2fc8f4657643e5b8003ea25b2c76146cee8baeeb8a8d8`. No migration or other function was deployed.
- ✅ **Automated evidence:** focused F12/BILLING-UNMAPPED `15/15`; security/invariants `875` total (`870` passed, `5` skipped, `0` failed); manual placement `59/59`; AutoPack Results `31/31`; full suite `988` total (`983` passed, `5` skipped, `0` failed). Edge/test syntax, typecheck, and diff checks passed; lint reported `0` errors with existing warnings only. Production-handler runtime tests cover monthly/annual direct siblings, different limits, newest direct outside another plan's slots, active/trialing/canceled, archived sibling, unknown price, coupon/zero-amount representation, duplicate direct rows, included/over-limit unpaid siblings, and archived requested organization. Function runtime covers conflicting local/Stripe binding; source-contract coverage preserves direct-first ordering, diagnostics, and unmapped-candidate behavior.
- ✅ **Available live-dev verification:** the directly paid `test1-Workspace` returned `active`, annual interval, its own current period end, price-derived workspace limit `3`, and `requestedDirectActiveCount: 1`. Its unpaid sibling `WS-test1` remained `included_in_plan` with the existing owner coverage list and `requestedDirectActiveCount: 0`. The unauthenticated error remained sanitized. No database row or Stripe object was changed.
- ⬜ **Fixture gaps — not represented as live passes:** no safe shared-dev fixture exists for two directly paid siblings, newest direct outside another plan's slots, active+trialing siblings, the exact paid+canceled sibling combination, archived paid winner plus active requested direct, coupon/100%-off, duplicate direct rows, conflicting metadata, or a four-workspace over-limit sibling. These paths passed production/function runtime coverage; no unsafe duplicate or conflict data was created.
- ⬜ **Still open and separate:** durable billing fixture implementation (the no-write safety foundation is complete at `3586daa`); plan/price catalog and unknown-price diagnostics; entitlement grant sources; coupons; additional tiers; complimentary and lifetime grants; database constraints/reconciliation; `getMyMembership` multi-workspace resolution; future Stripe customer/subscription transfer architecture; and Max Capacity Phase C. No catalog, tier, coupon, complimentary/lifetime-deal, client UI, AutoPack, or Phase C work was mixed into F12.

##### Billing fixture safety foundation — ✅ merged and pushed; no-write only (2026-07-14)
- ✅ **Implementation:** branch `chore/billing-fixture-safety-foundation`; commit/main integration tip `3586daa7161733176f105be941a4d0ff0b7806ca`. The foundation adds only CLI safety/planning modules, direct safety tests, operator documentation, two package scripts, and a fake test-mode environment placeholder. No production billing function, `src` runtime, migration, dependency, lockfile, or deployment changed.
- ✅ **Commands:** `npm run billing:fixtures:verify-safety` validates the environment; `npm run billing:fixtures:plan` validates and prints a masked in-memory fixture plan. Both perform zero network calls and zero writes. Future `seed`, `reset`, and `cleanup` names hard-fail with `Fixture writes are not implemented in the safety-foundation branch.`
- ✅ **Safety contract:** `TP3D_FIXTURE_ENV` must equal exact `dev`; `SUPABASE_URL` must contain a valid hosted project reference explicitly listed in `TP3D_FIXTURE_ALLOWED_PROJECT_REFS`; known production refs are refused even if allowlisted; an optional Stripe key must begin `sk_test_`; project display names are never accepted as authority; errors and diagnostics never echo URLs, keys, secrets, full IDs, or complete email addresses.
- ✅ **Manifest contract:** stable versioned fixture keys are bound to exact environment/project plus a unique run UUID. Organization/user/Stripe IDs remain null/empty in this no-write phase; eventual cleanup ownership is `manifest-ids-only`; validation rejects extra fields, duplicates, invalid versions/IDs, and cross-project/environment reuse. No personal email, secret, or F12 `includedOrgIds` expectation is stored.
- ✅ **Accurate test-layer inventory:** current coverage includes production function runtime with mocked dependencies, extracted production-helper runtime, synthetic runtime, source-contract/static checks, and manual browser/sandbox evidence. External Supabase and Stripe fixture integrations remain missing and are not claimed by this foundation.
- ✅ **Validation:** fixture safety `20/20`; security/invariants `875` total (`870` passed, `5` skipped, `0` failed); full suite `1008` total (`1003` passed, `5` skipped, `0` failed). New-file syntax, typecheck, lint (`0` errors; existing warnings only), and diff checks passed.
...
| ✅ | **Generic role updates cannot bypass ownership transfer.** `org-member-role-update` permits only member/admin transitions, rejects canonical or role-based owner mutations with `ownership_change_requires_transfer`, and leaves `organizations.owner_id` authoritative. | `91ffb51`; function v20; dev A–G PASS 2026-07-14 |
| ✅ | **F12 direct-paid identity is requested-workspace-first.** An unambiguous directly paid workspace uses its own subscription status, identity, interval, period, price-derived limit, and trial data before owner-plan coverage; duplicate/conflicting direct mappings fail closed, while unpaid sibling coverage remains unchanged. | `26ea600`; `billing-status` v72; available dev rows + production/function runtime coverage 2026-07-14 |
| ✅ | **Billing fixture safety foundation is complete and no-write.** Environment/project/Stripe-mode refusal, immutable manifest binding, masked diagnostics, and dry-run planning are implemented without a Supabase/Stripe client or external mutation. Durable local/dev/Stripe fixtures remain future branches. | `3586daa`; fixture safety `20/20`; full suite `1008` total (`1003` passed, `5` skipped) |
...
| 🟡 | Integration test: owner-account entitlement fallback does not bleed entitlement across workspaces — current direct/included/over-limit/trial/archived API matrix passes and F12 direct-first runtime coverage is green; exact metadata-less, multi-direct, duplicate/conflict, coupon, and four-workspace fixtures remain unavailable |
...
**Live editor visual-state cleanup — merged and pushed (`fix/editor-visual-highlight-cleanup`):** implementation/tests `6d91381`; documentation and main integration tip `779d4fd`. Browser PASS. The selection model remained correct; stale highlights came from selection, hover/drag, collision, and OOG paths mutating emissive/edge/opacity independently, including OOG cleanup skipping selected cases and rejected keyboard/Inspector moves retaining collision tint. `editor-screen.js` now recomputes each case from explicit current ownership using the deterministic priority **hidden → collision → selected → OOG → dragged/hovered → normal**. A committed scene sync clears preview-only collision ownership; rejected drag, keyboard nudge, and Inspector moves restore the owning selected/normal style; OOG refresh rebuilds the full OOG set and deterministically restores selected/normal emissive plus the original edge color. No materials or geometries are cloned during recompute. Final automated proof: `manual-vertical-placement` **59/59 passed** (4 direct visual-state/lifecycle tests); `security-and-invariants` **779 passed, 5 skipped, 0 failed**; full suite **892 passed, 5 skipped, 0 failed**; typecheck passed; lint **0 errors** (existing warnings only); diff and syntax checks passed. Manual browser verification passed in Standard, Wheel Wells, and Front Overhang: successful/rejected drag, OOG return, selected and non-selected restoration, multi-select, rotate/flip, nudge, Inspector move, dependent repair, hidden opacity, and repeated interactions all retained correct visual ownership with no stale material/edge accumulation and no console errors.
