...
|---|---|
| Repository | `main` is at `94dbfe4` (legacy-solver audit merged). `cleanup/remove-legacy-autopack-solver` holds the committed, unpushed legacy-solver removal (`src/services/autopack-legacy-solver.js` deleted, dependent tests/docs corrected; no production behavior changed). Inspector per-case Notes is the next active task, not yet started. |
...
| Legacy AutoPack solver removal | Closed. `src/services/autopack-legacy-solver.js` deleted on `cleanup/remove-legacy-autopack-solver` (committed, not yet pushed/merged); dependent tests redirected/trimmed and 2 stale docs corrected. No production behavior changed. |
...
| Field | Current value |
|---|---|
| Task | Implement Inspector per-case Notes. |
| Branch | Not yet created. |
| Outcome | Not yet started. |
| Blocker state | Unblocked. The legacy AutoPack solver removal that previously preceded this task in the queue is complete (`cleanup/remove-legacy-autopack-solver`, committed, pending push/merge). |
| Scope boundary | To be scoped when work begins. |
...
1. Push and merge `cleanup/remove-legacy-autopack-solver` to `main` (removal is complete and validated; merge is an operational step, not new work).
2. Implement Inspector per-case Notes.
3. Perform an `app.js` modularization inventory (M0 inventory only, not extraction) and plan low-risk extraction.
...
- None. The legacy AutoPack solver removal is complete and validated on `cleanup/remove-legacy-autopack-solver`, pending push/merge. Inspector per-case Notes is unblocked and not yet started.
...
The evidence-only legacy AutoPack solver retirement audit is closed and merged to `main` at `94dbfe4` with disposition SAFE TO REMOVE AFTER SMALL DEPENDENCY CLEANUP. Following that disposition, `src/services/autopack-legacy-solver.js` has been deleted on `cleanup/remove-legacy-autopack-solver` (committed and validated, pending push/merge); `buildLegacyAutoPackItems` is unaffected and remains owned by `autopack-item-builder.js`; no solver, engine, or editor behavior changed. Inspector per-case Notes is now the single active task (Section 4). See Section 7 for evidence links.
