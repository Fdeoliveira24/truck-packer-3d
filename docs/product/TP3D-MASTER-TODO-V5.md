...
|---|---|
| Repository | `main` is at `716c4d1` (legacy-solver removal merged; `src/services/autopack-legacy-solver.js` deleted, dependent tests/docs corrected; no production behavior changed). `refactor/app-js-p0-domain-cluster` (`bc9c6fd`) is the combined release branch: the complete, human-approved `app.js` P0 modularization (Billing/Organization/Auth/AccountSwitcher extractions, Composition Root no-op, final cleanup) stacked on Cargo Instructions Phase 0/1/2 (live QA complete) and Inspector case notes. It ships to `main` via one combined PR; `feat/inspector-case-notes` is not merged separately. Cargo Instructions Phase 3 is the next active task. |
...
| Field | Current value |
|---|---|
| Task | Cargo Instructions Phase 3 — Pack Notes Editor access. Add an Editor entry point opening a notes-only modal that calls `PackLibrary.update(pack.id, { notes })` directly, per the locked [Cargo Instructions Ownership Contract](../engineering/cargo-instructions-ownership-contract.md). |
| Branch | Not yet created. |
| Outcome | Not yet started. |
| Blocker state | Blocked until the combined release branch merges. Phase 3's predecessors — the complete `app.js` P0 modularization and Cargo Instructions Phase 1/2 (live QA complete) — are human-approved and shipping to `main` together via one combined PR from `refactor/app-js-p0-domain-cluster`; start Phase 3 on a fresh branch off the updated `main` after that merge. |
| Scope boundary | Pack Notes Editor access only. Do not reopen the frozen modularization boundaries (Billing/Organization/Auth/AccountSwitcher modules) or change billing, auth, workspace, or AutoPack behavior. |
...
1. Integrate the combined release branch `refactor/app-js-p0-domain-cluster` → `main` via one combined PR: the complete, human-approved `app.js` P0 modularization plus its stacked base — Cargo Instructions Phase 0/1/2 (live QA complete), Inspector case notes, and stabilization fixes. `feat/inspector-case-notes` is not merged separately.
2. Cargo Instructions Phase 3 (Pack Notes Editor access) — start on a fresh branch off the merged `main`, per the locked architecture in [Cargo Instructions Ownership Contract](../engineering/cargo-instructions-ownership-contract.md). Business identifiers (Section 12) remain a separate, later decision — not part of this queue item.
...
- No open blockers on the modularization / Cargo Instructions track. Cargo Instructions Phase 1/2 live browser QA is **complete** (2026-07-27, authenticated Chrome session against the local dev server); the earlier "no authenticated browser environment" blocker is resolved. The combined release branch `refactor/app-js-p0-domain-cluster` is human-approved and clear to merge to `main`.
