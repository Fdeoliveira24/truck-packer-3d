...
| `docs/billing/PRICING-OPERATIONS-RUNBOOK.md` | Billing operations | Active | Operational | Active | Maintain after every pricing architecture change | — | Current-state procedures only; not a commercial pricing decision or behavior contract. |
| `docs/billing/PRICING-CHANGE-LOG.md` | Billing operations | Active | Operational | Active | Append evidence after each approved pricing change | — | Header and reusable template only; no invented historical entries. |
...
| `docs/product/PROJECT_TREE.md` | Repository snapshot | Generated | No active authority | current | Regenerate mechanically after approved snapshot milestones; do not hand-edit paths | Current repository tree | Generated 2026-07-16; point-in-time reference only. |
...
| `docs/dev/billing-fixtures.md` | Developer operations/contract | Active | Fixture safety and staged implementation boundaries | authoritative | Maintain with each fixture milestone | — | The no-write foundation and local Stage B integration are complete. |
