...
12. Billing reliability, the Platform UX/UI Compatibility Closeout, Max Capacity Phase C reporting, the evidence-only AutoPack strategy differentiation audit, the evidence-only legacy AutoPack solver retirement audit, the legacy solver removal, the `app.js` P0 modularization, Cargo Instructions, customer-facing Load Plan terminology, and Business Identity Contract v1 are closed. Business Identity Phase 1 (Section 4) is the single active task.
...
| Field | Current value |
|---|---|
| Task | **Business Identity Phase 1 — Core Model and Migration.** Implement only the approved additive Case `itemCode`, Pack `loadPlanNumber`, Pack `customerReference`, normalization/validation primitives, canonical-load compatibility, and idempotent local migration defined by Business Identity Contract v1. |
| Branch | Not yet created. |
| Outcome | Not yet started. |
| Blocker state | Unblocked. Business Identity Contract v1 is completed and approved. |
| Scope boundary | Core model and migration only. No identifier UI, search/import/export/reporting integration, server-persistence uniqueness, packed-instance identity, or Future Extension work. Preserve existing UUIDs, internal Pack APIs/fields, JSON compatibility, local/offline persistence, duplication semantics, timestamps, ordering, and Undo/Redo behavior. |
...
1. Cases and Load Plans identifier UI.
2. Search, import, export, and reporting integration.
3. Server-persistence uniqueness contract.

Packed-instance identity and every Future Extension listed in Business Identity Contract v1 remain deferred.
...
- No open blockers. Business Identity Contract v1 is completed and approved; Business Identity Phase 1 may begin only on its separately created implementation branch.
...
[Business Identity Contract v1](../engineering/business-identity-contract-v1.md) is completed and approved. It is the permanent authority for technical, display, business, and external identity ownership and for v1 field, normalization, duplication, import, migration, presentation, and deferral rules.

- Implementation remains narrowly phased; only Business Identity Phase 1 — Core Model and Migration is Active Work.
- Cases and Load Plans identifier UI; search/import/export/reporting integration; and the server-persistence uniqueness contract remain separately queued.
